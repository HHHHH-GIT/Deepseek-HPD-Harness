...
1. 打开**设置 > 模型路由**。对话区用于发送消息，不提供按消息选择模型的功能。
2. 在已配置的模型目录中，分别为 **Light 模型**和 **Expert 模型**选择一个模型。Light 直接处理简单任务；Expert 规划并执行复杂任务。
3. 选择思维链强度模式：**Auto** 由每个已选模型自行决定，**Manual** 则为每个模型启用已配置的强度。修改会立即保存。配置完成后直接发起对话，H 会自动评估并路由每个任务。
