 ...
 *  - an explicit executable path, when configured;
 *  - Git Bash discovered beside the `git` executable on Windows; or
 *  - `bash` resolved from PATH on other platforms.
 *
...
/** Resolve an explicit shell or discover Git Bash beside the active Git executable. */
async function resolveBash(ctx, configuredPath, signal) {
  if (configuredPath !== undefined) {
    return await ctx.subprocess.resolveExecutable(configuredPath, undefined, signal)
  }
  ...
  try {
    ...
    return await ctx.subprocess.resolveExecutable(resolve(dirname(git), '..', 'bin', 'bash.exe'), undefined, signal)
  } catch (error) {
    throw new Error(`Git Bash was not found; configure custom-bash.bashPath: ${String(error)}`)
  }
}
...
export function apply(ctx, config) {
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      const shell = await resolveBash(ctx, bashPath, exec?.signal)
