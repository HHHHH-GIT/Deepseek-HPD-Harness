...
`agent/pre-step` decides what the model sees. Listeners may rewrite the claimed messages, replace the prepared prompt assembly for that step, or reject it outright; a rejected or empty first claim still closes a durable turn that spent no step, so the log records the attempt. Each step starts from the prompt sections and tool schemas that plugins registered.
